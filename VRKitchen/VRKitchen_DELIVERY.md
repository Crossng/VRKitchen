...
- Demo 会话组件会暴露全局厨房工位导览 `GetKitchenStationGuideText()`，说明面包台、蔬菜区、切菜板、调味区、煎锅/灶台、装盘区、出餐区和清理区分别做什么；该导览会进入玩家目标、订单板和教程文本，降低第一次进入 Demo 时的迷路成本。
- Demo 会话组件会暴露当前盘子装配引导 `GetCurrentPlateAssemblyGuideText()`，明确所有菜都要先在盘子上按顺序装好再提交；田园沙拉、牛排沙拉套餐和经典汉堡沙拉套餐会额外提示“沙拉也必须先在盘子上装好，再整盘提交到出餐区”。
...
- Demo 会话组件会暴露当前订单速查卡文本，提供更短的“当前菜品 / 食材 / 下一步 / 盘子装配 / 提交前检查 / 失败修复”信息，方便后续 VR 订单板或近距离提示牌显示。
...
- 田园沙拉已经作为正式菜单接入，要求把“切好的生菜, 切好的番茄, 沙拉酱”按顺序装到盘子上后整盘提交，不需要煎锅。
...
- `tools/verify_demo_menu_quality_via_bridge.py` 已加入交付门禁，用于只读检查 9 菜菜单健康度，确认解锁顺序、菜名唯一性、正式食材标签、中文玩家文案、切菜/煎锅/调味区提示、沙拉与套餐盘装规则都没有在后续改动中退化。
...
- Demo 菜单健康脚本 `tools/verify_demo_menu_quality_via_bridge.py` 已通过，确认菜单路线自检正常，菜名唯一、中文文案完整、沙拉/套餐盘装规则未退化。
...
- 自动化会检查全局厨房工位导览包含面包台、蔬菜区、切菜板、调味区、煎锅/灶台、装盘区、出餐区和清理区，并确认这些信息同时进入玩家目标、订单板和教程文本。
- 自动化会检查工位结果文本会随菜单阶段变化，并确认经典汉堡、香煎牛排、田园沙拉、牛排沙拉套餐和经典汉堡沙拉套餐的工位产出提示同时进入玩家目标、订单板和教程文本；沙拉相关订单必须提示调味区拿到沙拉酱，并明确在盘子上完成装配。
...
- 自动化会检查出餐前检查清单会随菜单阶段变化，并确认经典汉堡、香煎牛排、田园沙拉、牛排沙拉套餐和经典汉堡沙拉套餐的检查清单同时进入玩家目标文本和订单板详情文本；沙拉和沙拉套餐必须出现盘子装配检查项。
...
- 最近一次 Demo 地图内容验证时间：2026-05-31，命令行返回 `Success - 0 error(s), 0 warning(s)`。
